...
class SelfDrivingNode(Node):
    ...
    def set_led_right(self, blink_count=3, blink_interval=0.3):
        msg_on = RGBStates()
        msg_on.states = [RGBState(index=2, red=255, green=255, blue=0)]  # Yellow on
        msg_off = RGBStates()
        msg_off.states = [RGBState(index=2, red=0, green=0, blue=0)]  # Off
    ...
    def param_init(self):
        ...
        self.normal_speed = 0.2  #ormal driving speed
        ...
        while self.is_running:
            ...
            if self.start and self.button_state: 
                ...
                if self.start_slow_down:
                    ...
                    elif self.crosswalk_event_count == 1:
                        if self.crosswalk_distance > 155 and not self.crosswalk_stop:
                            self.get_logger().info("First crosswalk detected: Stopping")
                    ...
                    # elif self.crosswalk_event_count > 2:
                    #     self.crosswalk_event_count = 0
                ...
                if self.crosswalk_stop:
                    elapsed_time = time.time() - self.crosswalk_stop_start_time
                    if elapsed_time >= 2.0:  
                        self.crosswalk_stop = False
                        self.stop = False
                        self.get_logger().info("First crosswalk: Proceeding")
                ...
                if self.turn_right:
                    ...
                    if elapsed_time >= 2.0:  # 우회전 시간(테스트 필요)
                ...
                # result_image, lane_angle, lane_x = self.lane_detect(binary_image, image.copy())  # the coordinate of the line while the robot is in the middle of the lane
                    ...
                    if lane_x > 150:                          
                        ...
                        if self.count_turn > 5 and not self.start_turn:
                            self.crosswalk_event_count = 0
                            # self.get_logger().info("3333333333333333333333333333333333")
                            # self.get_logger().info('\033[1;33m%s\033[0m' % self.crosswalk_event_count)
