...
class SelfDrivingNode(Node):
    ...
    def handle_second_crosswalk(self):
        ...
        if self.traffic_light_status:
            if self.traffic_light_status.class_name == 'red':
                self.apply_gradual_stop()
                self.get_logger().info('적색 신호로 점진적 정지')
                return
            elif self.traffic_light_status.class_name == 'green':
    ...
    def main(self):
        while self.is_running:
            ...
            if self.start and self.button_state: 
                ...
                # elif self.crosswalk_distance > 70 and self.crosswalk_count < 2:
                ...
                # 주차 처리 (기존 로직 유지)
                if 0 < self.park_x and 135 < self.crosswalk_distance and self.crosswalk_count >= 2:
                    twist = Twist()
                    twist.linear.x = self.slow_down_speed
                    if not self.start_park and 180 < self.crosswalk_distance:
                        self.count_park += 1  
